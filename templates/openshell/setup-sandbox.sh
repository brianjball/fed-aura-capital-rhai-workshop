...
if [ -z "$MODELS" ]; then
    echo "Could not fetch models. Using default: gemma-4-31B-it-NVFP4"
    SELECTED="gemma-4-31B-it-NVFP4"
else
    i=1
    for m in $MODELS; do
        echo "  $i) $m"
    ...
    if [ -z "$SELECTED" ]; then
        echo "Invalid selection. Using default: gemma-4-31B-it-NVFP4"
        SELECTED="gemma-4-31B-it-NVFP4"
    fi
fi
...
RESULT=$(openshell sandbox exec --name "$SANDBOX_NAME" -- curl -s -w "\n%{http_code}" -X POST "${LITELLM_BASE_URL}/chat/completions" -H "Authorization: Bearer ${LITELLM_API_KEY}" -H "Content-Type: application/json" -d '{"model":"'"${LITELLM_MODEL:-gemma-4-31B-it-NVFP4}"'","messages":[{"role":"user","content":"Say ok"}],"max_tokens":5}' 2>&1 | grep -v "Using sandbox")
HTTP_CODE=$(echo "$RESULT" | tail -1)
if [ "$HTTP_CODE" = "200" ]; then
    info "LiteLLM test: OK (HTTP 200) - model: ${LITELLM_MODEL:-gemma-4-31B-it-NVFP4}"
else
...
echo "   opencode"
echo ""
echo " Model: ${LITELLM_MODEL:-gemma-4-31B-it-NVFP4}"
